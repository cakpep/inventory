...
<div class="row">
    ...
    <?= Html::a('<i class="glyphicon glyphicon-trash"></i> Hapus Seleksi', '#', [
                                                        ...
                                                        'onclick'=>'
                                                        var keys = $("#produk-bahan-table-grid").yiiGridView("getSelectedRows");                
    ...
    <?= Html::textInput("srch_all","",['placeholder'=>"Cari...",'onkeyup'=>'
                                                            $.pjax.reload({
                                                                url: "'.Url::to(['index']).'?ProdukBahanSearch[srch_all]="+$(this).val(),
                                                                container: "#produk-bahan-gridview",
                                                                timeout: 1000,
                                                            });
                                                            ','class'=>'form-control'])    ?>
    </div>        
    ...
    <?= Html::dropDownList('status', null, $data,['prompt'=>'- Cari Status','onchange'=>'
                                                            $.pjax.reload({
                                                                url: "'.Url::to(['index']).'?ProdukBahanSearch[status]="+$(this).val(),
                                                                container: "#produk-bahan-gridview",
                                                                timeout: 1000,
                                                            });
                                                            ','class'=>'form-control']) ?>
