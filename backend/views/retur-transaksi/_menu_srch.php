...
<div class="row">
    ...
    <?= Html::a('<i class="glyphicon glyphicon-trash"></i> Hapus Seleksi', '#', [
                                                        ...
                                                        'onclick'=>'
                                                        var keys = $("#transaksi-retur-table-grid").yiiGridView("getSelectedRows");                
    ...
    <?= Html::textInput("srch_all","",['placeholder'=>"Cari...",'onkeyup'=>'
                                                            $.pjax.reload({
                                                                url: "'.Url::to(['index']).'?TransaksiReturSearch[srch_all]="+$(this).val(),
                                                                container: "#transaksi-retur-gridview",
                                                                timeout: 1000,
                                                            });
                                                            ','class'=>'form-control'])    ?>
    </div>        
    ...
    <?= Html::dropDownList('status', null, $data,['prompt'=>'- Cari Status','onchange'=>'
                                                            $.pjax.reload({
                                                                url: "'.Url::to(['index']).'?TransaksiReturSearch[status]="+$(this).val(),
                                                                container: "#transaksi-retur-gridview",
                                                                timeout: 1000,
                                                            });
                                                            ','class'=>'form-control']) ?>
